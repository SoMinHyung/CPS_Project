...
### 시행착오
1. 주제선정
> 맨 처음에 주제의 방향성을 정하는데 어려움이 있었다. 다크웹에서 IoT hacking에 관한 글들을 크롤링해서, 이를 감성분석해보자는 의견이 있었다. 하지만 토르브라우저의 접속문제, 다크웹의 속도문제, 데이터 유효성에 의문이 생겨서 해당 아이디어는 기각되었다.
<br/>

2. 구현과정
> 1. 데이터 수집: IoT security 관련 글이 많을 것이라고 예상했지만 커뮤니티 내에는 전문가 포스팅, 뉴스 url 등 전문가 데이터가 많았고, 관련 뉴스 기사에는 댓글이 없어 수집 데이터가 양적으로, 질적으로 부족했다. 이에 추가적으로 설문조사를 진행하여, 많은 응답을 받진 못했지만 IoT에 대해 알지 못하는 사람이 많다는 유의미한 데이터를 얻었다

> 2. 크롤러 개발 : 자바스크립트를 사용하면서, 일반적인 방식의 크롤러의 접근방식이 불가능한 사이트를 크롤링해야할 필요가 있었다. 며칠동안 에러로 인해 개발에 실패하는 상황이어서, 해당 사이트의 정보 수집을 포기할까?라는 의견이 조에서 나왔다. 하지만 Quora에 로그인 후 사용이 가능한 서비스인데, 크롤러를 통한 로그인이 불가능해서 조교님에게 문의해 해결하였다. 

> 3. 딥러닝 모델링 : 데이터의 양이 너무 많아서, tensorflow에 입력할 input_data의 사이즈를 맞춰주는 것이 꽤나 어려웠다. ngram와 tfidf 모델에서 데이터 전처리를 하면서 사이즈를 조정하는 방식으로 input_size를 동일하게 맞춰줄 수 있었다.

> 4. 데이터 분석 : 분석 결과로 나온 값이 우리가 예상한 값과 많이 달라서 당황했다. 해당 오차가 발생한 원인이 무엇인지 분석하고, 도출된 결론으로부터 우리가 얻을 수 있는 인사이트가 무엇인지 알아보는데 많은 설왕설래가 있었다. 통계적 지식이 부족해서 결과 해석에서도 어려움이 있었다.
<br/>
...
5. 딥러닝 기반의 감성분석 모델을 토대로 다양한 분야의 소비자 감성 분석 등에 적용 가능하다. 
<br/>

6. 추후에 보안 사고가 발생하거나 특정 이슈가 발생했을 때, 이 모델로 감성분석을 한다면 사고의 impact, 중요성을 알 수 있는 척도가 될 수 있다.
>Ex) 현재 평균 : 40점<br/>
...
### 유나은
프로젝트를 시작할 땐, 파이썬을 이번 학기에 처음 접하고 크롤링 과제를 하면서도 많이 헤매 과연 프로젝트를 성공적으로 진행을 할 수 있을지, 민폐가 되는 것은 아닐지 걱정이 컸다. 사실 코딩이라고 하면 피하고 싶은, 어려운 것이라고 생각했는데 보안의식 조사라는 흥미로운 주제를 갖고 팀원분들과 함께 수행을 하면서 재밌게 진행을 한 것 같다. 

보안 공부를 시작하며 ‘보안 의식’에 대해서 많이 다뤘지만 와닿지는 않았었는데 이번 프로젝트를 통해 IoT, 보안 등에 대해 일반사용자의 관심이 크지 않다는 것, 그리고 비용이나 현 정책 등의 부족함에 대해서는 부정적이지만 필요성이나 중요성을 인식하고 있다는 것을 직접적으로 느낄 수 있어 흥미로웠다. 또한 데이터 분석에 대해서는 언뜻 들어본 적은 있지만, 정확히 어떻게 진행하고 언제 사용되는지는 몰랐는데 이번 프로젝트에서 데이터 전처리와 데이터 분석에 대해 공부하면서 많이 알게 되었다. 특히 ngram을 이용해 두 단어를 묶어 문맥상 의미를 반영하고, tf-idf 가중치를 통해 문서에서 중요한 핵심어, 상투어를 구분할 수 있다는 것이 흥미로웠다. 그리고 물론 데이터 전처리, 분석 과정도 중요하지만 수행 과정에서 수집 데이터의 양과 질이 근본적으로 가장 중요한 것이라는 것을 느꼈다. 

혼자서는 수행하지 못했을 프로제트를 팀원들에게 배우고, 함께 생각이나 공유를 하는 과정을 거쳐 끝내고 나니 뿌듯함과 감사함이 크다. 향후 코딩 실력을 키워 이번에는 실력이 부족해 이해에서 그쳤던 것을 직접 작성할 수 있도록 하고 싶다.
